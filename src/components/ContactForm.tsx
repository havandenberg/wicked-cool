...
const ErrorMessage = styled(l.CenteredRow)({color: colors.red, fontSize: fontSizes.largeText});
...
type contactField = 'name' | 'email' | 'phone' | 'message' | 'serviceDate' | 'serviceTime';
...
State > {
  ...
  handleSubmit = (e : React.FormEvent) => {
    ...
    if (!loading && this.validate()) {
      this.setState({
        ...
        emailjs.send('wicked_cool_gmail', 'template_YTh9UKQv', {
          'from_name': name,
          'message': message,
          'phone': phone,
          'reply_to': email,
        ...
        }, 'user_x41YJeHB2mv7uIY1xl7sw').then(() => {
  ...
  hasErrors = () => {
    ...
    return errors.name || errors.email || errors.phone || errors.serviceDate || errors.serviceTime;
  }
  ...
  render() {
    ...
    return (
      <form onSubmit={this.handleSubmit}>
        <ContactFormWrapper>
          {completed && !failed && <t.Text center fontSize={fontSizes.largeText}>Success! Thank you for your
            request and we will get back to you as soon as possible.</t.Text>}
          {!completed && failed && <t.Text center color={colors.red} fontSize={fontSizes.largeText}>An error has occurred. Please try again later or email us directly at
            <t.Anchor
          ...
          {!completed && !failed && <div>
            {this.hasErrors() && <ErrorMessage mb={spacing.xl}>Please correct the fields highlighted below and try again.</ErrorMessage>}
            <InputWrapper>
              <InputLabel error={errors.name} nowrap>Name or company*:</InputLabel>
              <Input error={errors.name} onChange={this.handleChange('name')} value={name}/>
            </InputWrapper>
            <InputWrapper>
            ...
            <InputWrapper alignTop>
              <InputLabel error={errors.message} mt={[0, spacing.s]}>Message*:</InputLabel>
              <TextArea
            ...
            {action === SERVICE_ACTION && <InputWrapper>
              <InputLabel error={errors.serviceDate}>Service Date*:</InputLabel>
              <Input
            ...
            {action === SERVICE_ACTION && <InputWrapper>
              <InputLabel error={errors.serviceTime} mt={spacing.s}>Service Time*:</InputLabel>
              <Input
        ...
        {!completed && !failed && <l.CenteredRow mt={spacing.xl}>
          {loading
            ? <PulseLoader sizeUnit="px" size={30} color={colors.darkBlue}/>
            : <Submit type="submit"><SendIcon src={SendImg}/>Submit</Submit>}
        </l.CenteredRow>}
        {(completed || failed) && <l.CenteredRow mt={spacing.xl}>
